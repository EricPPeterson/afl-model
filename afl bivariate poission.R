...
sched_2023 <- sched_2023_all %>% filter(round.roundNumber <= 15)
...
current_week <- fetch_fixture(season = 2023) %>% 
  ...
  separate(utcStartTime, c('Date', 'Time'), 'T') %>%
  separate(Time, c('Hour', 'Minute', 'Second'), ':') %>%
  select(-c('Second')) %>% filter(round.roundNumber == 19)
...
write.csv(final_df_new, 'final_df_19.csv', row.names = FALSE)
